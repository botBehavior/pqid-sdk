...
export const cryptoApi = {
  ...
  async derive_key(password: Uint8Array, salt: Uint8Array, iterations: number): Promise<Uint8Array> {
  ...
  async aes_gcm_encrypt(plaintext: Uint8Array, key: Uint8Array, nonce: Uint8Array): Promise<Uint8Array> {
    const subtle = getSubtle();
  ...
  async aes_gcm_decrypt(ciphertext: Uint8Array, key: Uint8Array): Promise<Uint8Array> {
    const subtle = getSubtle();
  ...
  async ml_dsa_keygen(): Promise<Uint8Array> {
    await ensureCryptoReady();
    const { publicKey, secretKey } = await ml_dsa65.keygen();
    return concatBytes(publicKey, secretKey);
  },

  async ml_dsa_sign(message: Uint8Array, secretKey: Uint8Array): Promise<Uint8Array> {
    await ensureCryptoReady();
    return ml_dsa65.sign(message, secretKey);
  },

  async ml_dsa_verify(message: Uint8Array, signature: Uint8Array, publicKey: Uint8Array): Promise<boolean> {
    await ensureCryptoReady();
    return ml_dsa65.verify(signature, message, publicKey);
  },

  /** FIPS 203 ML-KEM-768 key generation. Returns publicKey || secretKey. */
  async ml_kem_keygen(): Promise<Uint8Array> {
    await ensureCryptoReady();
    const { publicKey, secretKey } = await ml_kem768.keygen();
    return concatBytes(publicKey, secretKey);
  },

  async ml_kem_encapsulate(publicKey: Uint8Array): Promise<Uint8Array> {
    await ensureCryptoReady();
  ...
  async ml_kem_decapsulate(ciphertext: Uint8Array, secretKey: Uint8Array): Promise<Uint8Array> {
    await ensureCryptoReady();
