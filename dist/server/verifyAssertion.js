...
function pickAuthenticationMethod(bundle) {
    var _a;
    ...
    const preferredId = (_a = didDocument.authentication) === null || _a === void 0 ? void 0 : _a[0];
    if (preferredId) {
        return verificationMethods.find((method) => method.id === preferredId);
    }
    return verificationMethods[0];
}
export async function verifyAssertion(bundle) {
    if (!bundle) {
    ...
    const verified = await verify({ publicKey: publicKeyBase64, algorithm }, canonical, bundle.assertion_signatureBase64);
    if (!verified) {
